...
export async function runSlaCheck(): Promise<{ checked: number; notified: number; errors: number }> {
  ...
  try {
    ...
    const orders = await prisma.order.findMany({
      ...
      select: {
        id: true, orderNum: true, createdAt: true, managerId: true,
        organizationId: true,
    ...
    for (const order of orders) {
      try {
        ...
        await logActivity({
          organizationId: order.organizationId,
          action: 'SLA_OVERDUE', entityType: 'Order', entityId: order.id,
...
export function startSlaTracker() {
  ...
  slaJob = cron.schedule(SLA_CRON, async () => { await runSlaCheck(); });
  logger.info('SLA Tracker: started');
}

export function stopSlaTracker() {
  if (slaJob) { slaJob.stop(); slaJob = null; logger.info('SLA Tracker: stopped'); }
}
